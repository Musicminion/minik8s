...
**项目架构**：我们的项目架构学习了K8s官方的架构，同时又适应需求做了一定的微调。整体主要是由控制平面和Worker节点两大类组成。
...
- 所有的日常代码的推送都会被发送到我们自己的服务器，运行单元测试，并直接该推送的结果
- 当发起PR时，会再一次运行单元测试，测试通过之后才可以合并
...
- 我们的软件开发基于迭代开发、敏捷开发。小组成员每天晚上在软件学院大楼集中进行开发新功能，减少沟通障碍，做到有问题及时解决、沟通，有困难相互请教，这也大大的提高了我们小组的效率。截止15周周末，我们已经完成了所有的功能的开发。基本符合预期进度。
...
- Replica Controller：维护Replica的数量和期望的数量一致，如果出现数量不一致，当通过标签匹配到的Pod数量较多的时候，会随机的杀掉若干Pod，直到数量和期望一致；当通过标签匹配到的Pod数量偏少的时候，会根据template创建相关的Pod
...
- `kubectl execute [namespace]/[name] [parameters]` 触发一个Serveless的函数，并传递相关的参数
...
Kubeproxy运行在每个Worker节点上，主要是为了支持service抽象，以实现根据Service ClusterIP访问Pod服务的功能，同时提供了一定负载均衡功能，例如可以通过随机或者轮询的策略进行流量的转发。同时Kubeproxy还承担了DNS和转发的功能。
...
Pod是k8s(minik8s)调度的最小单位。用户可以通过 `Kubectl apply Podfile.yaml` 的声明式的方法创建一个Pod。当用户执行该命令后，Kubectl会将创建Pod的请求发送给API-Server。API-Server检查新创建的Pod在格式、字段是否存在问题，如果没有异常，就会写入Etcd，并给Scheduler发送请求调度的消息。

Scheduler完成调度之后，会向API-Server发送HTTP请求更新pod的nodename，并向对应的节点的Kubelet发送创建Pod的消息。
...
Pod创建之后，Kubelet的Status Manager会不断监视Pod的运行状态，并将状态更新写回到API-Server(通过Pod的Status的接口)。如果Pod中的容器发⽣崩溃或⾃⾏终⽌，首先PlegManager会通过StatusManager捕捉到Pod的异常状态，然后会产生Pod生命周期事件，通过PlegChan发送需要重启Pod的命令。然后WorkerManager收到之后会执行重启的操作。
