...
func GetNode(c *gin.Context) {
	...
	// 如果解析成功，返回对应的Node信息
	if name != "" {
		res, err := EtcdStore.PrefixGet(config.EtcdNodePath + name)
		if err != nil {
			c.JSON(400, gin.H{
...
func GetNodes(c *gin.Context) {
	res, err := EtcdStore.PrefixGet(config.EtcdNodePath)
	if err != nil {
		c.JSON(400, gin.H{
...
func DeleteNode(c *gin.Context) {
	...
	if name != "" {
		...
		err := EtcdStore.Del(config.EtcdNodePath + name)
		if err != nil {
			c.JSON(400, gin.H{
...
func AddNode(c *gin.Context) {
	// log
	k8log.InfoLog("AddNode: add new node")
	// POST请求，获取请求体
	...
	if err := c.ShouldBind(&node); err != nil {
		...
		k8log.ErrorLog("AddNode: parser node failed " + err.Error())
		return
	}

	// 检查name是否重复
	res, err := EtcdStore.PrefixGet(config.EtcdNodePath + node.NodeMetadata.Name)
	if err != nil {
		c.JSON(500, gin.H{
			"error": "get node failed " + err.Error(),
		})
		k8log.ErrorLog("AddNode: get node failed " + err.Error())
		return
	}
	...
	if len(res) != 0 {
		...
		k8log.ErrorLog("AddNode: node name already exist")
		return
	}
	...
	// 将Node信息写入etcd
	err = EtcdStore.Put(config.EtcdNodePath+node.NodeMetadata.Name, nodeJson)
	if err != nil {
		c.JSON(500, gin.H{
